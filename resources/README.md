# Resources

A curated collection of resources for learning about Agentic AI and Semantic Kernel.

## 📖 Official Documentation

### Semantic Kernel
- [Semantic Kernel Overview](https://learn.microsoft.com/en-us/semantic-kernel/overview/)
- [Getting Started Guide](https://learn.microsoft.com/en-us/semantic-kernel/get-started/quick-start-guide)
- [Concepts and Architecture](https://learn.microsoft.com/en-us/semantic-kernel/concepts/)
- [API Reference](https://learn.microsoft.com/en-us/semantic-kernel/api-reference/)

### Azure OpenAI
- [Azure OpenAI Service](https://azure.microsoft.com/en-us/products/ai-services/openai-service)
- [Azure OpenAI Documentation](https://learn.microsoft.com/en-us/azure/ai-services/openai/)
- [Quickstart: Get Started](https://learn.microsoft.com/en-us/azure/ai-services/openai/quickstart)

### OpenAI
- [OpenAI Platform Documentation](https://platform.openai.com/docs/)
- [OpenAI API Reference](https://platform.openai.com/docs/api-reference)
- [GPT Best Practices](https://platform.openai.com/docs/guides/gpt-best-practices)

## 🎓 Learning Materials

### Tutorials and Guides
- [Build Your First AI Agent with Semantic Kernel](https://learn.microsoft.com/en-us/semantic-kernel/get-started/)
- [Understanding AI Agents](https://learn.microsoft.com/en-us/semantic-kernel/agents/)
- [Creating Plugins](https://learn.microsoft.com/en-us/semantic-kernel/agents/plugins/)
- [Working with Memory](https://learn.microsoft.com/en-us/semantic-kernel/memories/)
- [Using Planners](https://learn.microsoft.com/en-us/semantic-kernel/agents/planners/)

### Video Content
- [Semantic Kernel Introduction](https://www.youtube.com/watch?v=pJMZRBfRQeg)
- [Building AI Agents - Microsoft Build](https://build.microsoft.com/)
- [Semantic Kernel YouTube Channel](https://www.youtube.com/@SemanticKernel)

### Blog Posts
- [Microsoft AI Blog](https://blogs.microsoft.com/ai/)
- [Semantic Kernel Blog](https://devblogs.microsoft.com/semantic-kernel/)
- [Azure AI Blog](https://techcommunity.microsoft.com/t5/azure-ai-services-blog/bg-p/Azure-AI-Services-blog)

## 🛠️ Code Repositories

### Official Examples
- [Semantic Kernel Samples (Python)](https://github.com/microsoft/semantic-kernel/tree/main/python/samples)
- [Semantic Kernel Samples (C#)](https://github.com/microsoft/semantic-kernel/tree/main/dotnet/samples)
- [Semantic Kernel Samples (Java)](https://github.com/microsoft/semantic-kernel/tree/main/java/samples)

### Community Projects
- [Semantic Kernel Awesome List](https://github.com/microsoft/semantic-kernel/discussions/categories/show-and-tell)
- [Community Plugins](https://github.com/topics/semantic-kernel)

## 📚 Books and Papers

### Recommended Reading
- "Building LLM Powered Applications" - Microsoft Learn
- "Generative AI with Large Language Models" - DeepLearning.AI
- "Prompt Engineering Guide" - DAIR.AI

### Research Papers
- [ReAct: Synergizing Reasoning and Acting in Language Models](https://arxiv.org/abs/2210.03629)
- [Agents: An Open-source Framework for Autonomous AI](https://arxiv.org/abs/2308.08155)
- [AutoGPT: An Autonomous GPT-4 Experiment](https://github.com/Significant-Gravitas/AutoGPT)

## 🌐 Community

### Discussion Forums
- [Semantic Kernel Discord](https://aka.ms/SKDiscord)
- [Semantic Kernel GitHub Discussions](https://github.com/microsoft/semantic-kernel/discussions)
- [Azure AI Community](https://techcommunity.microsoft.com/t5/azure-ai-services/ct-p/Azure-AI-Services)

### Social Media
- [Semantic Kernel Twitter](https://twitter.com/semantickernel)
- [Microsoft AI Twitter](https://twitter.com/MSFTAIDev)

## 🔧 Tools and Libraries

### Development Tools
- [Visual Studio Code](https://code.visualstudio.com/)
- [GitHub Copilot](https://github.com/features/copilot)
- [Jupyter Notebooks](https://jupyter.org/)

### Related Frameworks
- [LangChain](https://python.langchain.com/) - Alternative AI framework
- [AutoGen](https://microsoft.github.io/autogen/) - Multi-agent framework
- [LlamaIndex](https://www.llamaindex.ai/) - Data framework for LLMs

### Testing Tools
- [Prompt Flow](https://microsoft.github.io/promptflow/) - LLM application development
- [Azure AI Studio](https://azure.microsoft.com/en-us/products/ai-studio/)

## 📊 Use Case Examples

### Industry Applications
- **Customer Service**: AI chatbots with tool access
- **Development**: Code generation and review agents
- **Research**: Information gathering and synthesis
- **Data Analysis**: Automated data processing and insights
- **Task Automation**: Workflow orchestration agents

### Sample Projects
- [Customer Support Bot with Semantic Kernel](https://github.com/microsoft/semantic-kernel-starters)
- [Code Review Assistant](https://github.com/microsoft/semantic-kernel/tree/main/samples/apps)
- [Research Assistant](https://github.com/microsoft/semantic-kernel/tree/main/samples/apps)

## 🎯 Hackathon Tips

### Best Practices
- Start with simple examples and iterate
- Focus on one clear use case
- Test with different prompts and inputs
- Monitor token usage and costs
- Document your approach and learnings

### Common Pitfalls to Avoid
- Overcomplicating the initial design
- Not handling API errors properly
- Ignoring token limits
- Hardcoding API keys
- Skipping proper testing

## 📝 Cheat Sheets

### Quick References
- [Semantic Kernel Cheat Sheet (Python)](https://github.com/microsoft/semantic-kernel/wiki)
- [Prompt Engineering Guide](https://www.promptingguide.ai/)
- [Azure OpenAI Models Comparison](https://learn.microsoft.com/en-us/azure/ai-services/openai/concepts/models)

## 🆘 Getting Help

### Support Channels
- Hack event organizers and mentors
- Semantic Kernel Discord community
- GitHub Issues for bugs
- Stack Overflow for technical questions

### FAQ
Check the [Semantic Kernel FAQ](https://github.com/microsoft/semantic-kernel/discussions/categories/q-a) for common questions and answers.

---

This list is continuously updated. Feel free to suggest additions via pull request!
